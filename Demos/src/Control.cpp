...
void Control::moveArmToWorldPoint(std::vector<Vector3D> pose){
    ...
    while(!Arm::getArm()->amAtPoint(pos.x, pos.y, pos.z,
        ...
        std::cout << "NOT at point" << pos.x << " " << pos.y << " " << pos.z << std::endl;
        Arm::getArm()->moveToPoint(pos.x, pos.y, pos.z,
                               euler_angles.at(0),
                               euler_angles.at(1),
                               euler_angles.at(2));
...
void Control::moveArmToWorldPoint(Vector3D point){
    point = Common::worldPointToArmSpace(point);
    point.scale(0.1f);
    ...
    std::vector<float> pose_eurler_angles = eulerAnglesFromPose(ik_result.result_pose);

    ik_result.result_pose[0].z += 3.0f;
    ik_result.result_pose[0].scale(10.0f);
    ik_result.result_pose[0].print();

    Arm::getArm()->moveToPoint(ik_result.result_pose[0].x, ik_result.result_pose[0].y, ik_result.result_pose[0].z,
    						   pose_eurler_angles[0], pose_eurler_angles[1], pose_eurler_angles[2]);
...
void Control::tiltWritst(float angle){
	 std::vector<float> cur_joints = Arm::getArm()->requestCurrentAngles();
	 ArmForwardKinematics fk;
	 std::vector<Vector3D> cur_pose = fk.getArmPoint(cur_joints);

	 std::vector<Vector3D> rot_mat = Geometry::axisRotationMatrix(cur_pose[3], -32.0f*M_PI/180.0f);
	 Vector3D offset_vec = cur_pose[1];
	 offset_vec.matrixMultLeft(rot_mat);
	 Vector3D rot_axis = offset_vec;
	 rot_axis.normalise();

	 std::vector<Vector3D> rot_mat2 = Geometry::axisRotationMatrix(rot_axis, angle*M_PI/180.0f);
	 cur_pose[1].matrixMultLeft(rot_mat2);
	 cur_pose[2].matrixMultLeft(rot_mat2);
	 cur_pose[3].matrixMultLeft(rot_mat2);

	 std::vector<float> pose_eurler_angles = eulerAnglesFromPose(cur_pose);
	 Arm::getArm()->moveToPoint(cur_pose[0].x, cur_pose[0].y, cur_pose[0].z,
								pose_eurler_angles[0], pose_eurler_angles[1], pose_eurler_angles[2]);

}

void Control::throwObject(){
    std::vector<float> joints;
    joints.push_back(83.0f);
    joints.push_back(32.0f);
    joints.push_back(113.0f);
    joints.push_back(94.0f);
    joints.push_back(-54.0f);
    ...
    joints.clear();
    joints.push_back(73.0f);
    joints.push_back(31.0f);
    joints.push_back(107.0f);
    joints.push_back(92.0f);
    joints.push_back(-100.0f);
    joints.push_back(-80.0f);
    Arm::getArm()->openHand(1.0f);
