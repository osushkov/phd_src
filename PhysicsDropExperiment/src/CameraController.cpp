...
bool CameraController::getNewFrame(void){
    ...
    */
    ...
    std::vector<unsigned int> avrg_left_cam(1024*768*3, 0), avrg_right_cam(1024*768*3, 0);
    unsigned num_avrg = 2;
    for(unsigned i = 0; i < num_avrg; i++){
        cam->getImage(left_cam_buffer, right_cam_buffer);
        for(unsigned j = 0; j < left_cam_buffer.size(); j++){
            avrg_left_cam[j] += (unsigned int)left_cam_buffer[j];
        }
        for(unsigned j = 0; j < right_cam_buffer.size(); j++){
            avrg_right_cam[j] += (unsigned int)right_cam_buffer[j];
        }
    }

    for(unsigned j = 0; j < left_cam_buffer.size(); j++){
        left_cam_buffer[j] = (unsigned char)(avrg_left_cam[j]/num_avrg);
    }
    for(unsigned j = 0; j < right_cam_buffer.size(); j++){
        right_cam_buffer[j] = (unsigned char)(avrg_right_cam[j]/num_avrg);
    }
    ...
    if (!paused && gen_sift) {
        ...
        for(unsigned i = 0; i < right_sift_features.size(); i++){
            right_sift_features[i].y += 2.5f; // DODGY HACK because the cameras are not 100% perfectly aligned in the Y direction.
        }
