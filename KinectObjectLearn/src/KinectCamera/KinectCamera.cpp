...
bool KinectCamera::getCorrelatedImage(KinectCamera *cam, KinectCamera::CorrelatedImage &result, bool blocking) {
    ...
    if(!cam->getImage(lrgb_buffer, ldepth_buffer, blocking)){
        return false;
    }
    ...
    const int x_window = 4;
    const int y_window = 4;
...
Vector2D KinectCamera::worldSpaceToRGBPixelCoord(Vector3D coord){
    ...
    Vector2D result;
    result.x = (coord.x * fx_rgb * invZ) + cx_rgb - 1.0f;
    result.y = (coord.y * fy_rgb * invZ) + cy_rgb - 1.0f; // TODO: dodgy hack this is, myeeees.
    return result;
}