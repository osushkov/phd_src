...
void RawPointCloud::generate(ObjectSnapshotDB *object_snapshot_db, 
                             std::string video_path, std::string sift_path){
    CameraController camera_controller(video_path, sift_path);
    ...
    while (camera_controller.isActive()) {
        ...
        std::vector<SIFTFeature3D> frame_features = camera_controller.getStereoFeatures();
        ...
        std::vector<SIFTFeature3D> remaining_features;
        *(char*)NULL = 5;
        /*
        ...
        */
        ...
        std::vector<SIFTFeature3D> new_object_features, more_arm_features;
...
void RawPointCloud::renderDetectedPoints(CameraController &camera_controller,
    ...
    // Render the features.
    IplImage *img = Common::imageFromCorrelatedFrame(camera_controller.getCorrelatedFrame());                                             

    for (unsigned i = 0; i < point_cloud_frame.object_features.size(); i++) {
        CvPoint pos = cvPoint(point_cloud_frame.object_features[i].sift_feature.x,
                               point_cloud_frame.object_features[i].sift_feature.y);

        cvCircle(img, pos, 1, CV_RGB(0, 255, 0), 2.0);
    }

    for (unsigned i = 0; i < point_cloud_frame.arm_features.size(); i++) {
        CvPoint pos = cvPoint(point_cloud_frame.arm_features[i].sift_feature.x,
                               point_cloud_frame.arm_features[i].sift_feature.y);

        cvCircle(img, pos, 1, CV_RGB(0, 0, 255), 2.0);
    }
}

void RawPointCloud::partitionFeatureSet(const std::vector<SIFTFeature3D> &start_features,
                                        float sd_threshold,
