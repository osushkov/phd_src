...
float SuperQuadric::F(Vector3D p) const {
    float a = pow(fabs(p.x/A), 2.0f/e2);
    float b = pow(fabs(p.y/B), 2.0f/e2);
    float c = pow(a + b, e2/e1);
    float d = pow(fabs(p.z/C), 2.0f/e1);
    return c + d;
}

float SuperQuadric::distance(Vector3D p) const {
    float zp = (p.z+C)/(2.0f*C);
    if(zp < 0.0f){ zp = 0.0f; }
    if(zp > 1.0f){ zp = 1.0f; }
    ...
    return p.length() * (1.0f - powf(F(p), -e1/2.0f));// * mod;
    //float d = p.length()/pow(F(p), e1/2.0);
    //return d;
}

float SuperQuadric::distance2(Vector3D p) const {
    //return distance(p);
    return findClosestSurfacePointDist2(p);
}

float SuperQuadric::distance3(Vector3D p) const {
    float mod = xt + (1.0f-xt)*(p.z+C)/(2.0f*C);
    p.x /= mod;
    p.y /= mod;

    return p.length()/pow(F(p), e1/2.0f);
/*
...
SuperQuadric::SuperQuadric(float e1, float e2, float A, float B, float C) :
    e1(e1), e2(e2), A(A), B(B), C(C), xt(1.0), /*yt(1.0),*/ set(true),
    udivs(20), vdivs(20), uinc(2.0f*(float)M_PI/udivs), vinc((float)M_PI/vdivs) {

    //generateSurfacePoints();
}

SuperQuadric::SuperQuadric(float e1, float e2, float A, float B, float C, float xt, float yt) :
...
SuperQuadric::SuperQuadric(float e1, float e2, float A, float B, float C, float xt, float yt, unsigned mesh_res) :
    e1(e1), e2(e2), A(A), B(B), C(C), xt(xt), /*yt(yt),*/ set(true),
    udivs(mesh_res), vdivs(mesh_res), uinc(2.0f*(float)M_PI/udivs), vinc((float)M_PI/vdivs) {

    generateSurfacePoints();
}

SuperQuadric::SuperQuadric() :
    set(false), udivs(20), vdivs(20), uinc(2.0f*(float)M_PI/udivs), vinc((float)M_PI/vdivs) {
...
bool SuperQuadric::load(std::istream &in) {
    ...
    in.read((char*)&e1, sizeof(float));
    in.read((char*)&e2, sizeof(float));

    in.read((char*)&A, sizeof(float));
    in.read((char*)&B, sizeof(float));
    in.read((char*)&C, sizeof(float));

    in.read((char*)&xt, sizeof(float));
    set = true;
...
bool SuperQuadric::save(std::ostream &out){
    out.write((char*)&e1, sizeof(float));
    out.write((char*)&e2, sizeof(float));

    out.write((char*)&A, sizeof(float));
    out.write((char*)&B, sizeof(float));
    out.write((char*)&C, sizeof(float));

    out.write((char*)&xt, sizeof(float));
...
Vector3D SuperQuadric::getSurfacePosition(float u, float v) const {
    Vector3D result;
...
Vector3D SuperQuadric::getSurfaceNormal(float u, float v) const {
    Vector3D result;
    result.x = (float)((1.0f/A)*sign(cosf(u)*cosf(v))*powf(fabs(cosf(v)), 2.0-e1)*powf(fabs(cosf(u)), 2.0f-e2));
    result.y = (float)((1.0f/B)*sign(sinf(u)*cos(v))*powf(fabs(cosf(v)), 2.0-e1)*powf(fabs(sinf(u)), 2.0f-e2));
    result.z = (float)((1.0f/C)*sign(sinf(v))*powf(fabs(sinf(v)), 2.0-e1));
    result.normalise();
/*
...
/*
    ...
    while(result.vertices.size() < 400){
        float max_size = (float)std::max<float>(A,std::max<float>(B,C));
...
float SuperQuadric::sign(float x) const {
...
/*
        ...
        pos.normalise();*/
            ...
            float x = max_size*sinf(v)*cosf(u);
            float y = max_size*sinf(v)*sinf(u);
            float z = max_size*cosf(v);
...
void SuperQuadric::getUVLimits(Vector3D p, std::pair<float,float> &ulimits, std::pair<float,float> &vlimits) const {
    if(p.z >= 0.0f){
        vlimits.first = 0.0f;
        vlimits.second = (float)M_PI/2.0f;
    }
    else{
        vlimits.first = -(float)M_PI/2.0f;
        vlimits.second = 0.0f;
    }

    if(p.x >= 0.0f && p.y >= 0.0f){
        ulimits.first = 0.0f;
        ulimits.second = (float)M_PI/2.0f;
    }
    else if(p.x < 0 && p.y >= 0.0f){
        ulimits.first = (float)M_PI/2.0f;
        ulimits.second = (float)M_PI;
    }
...
Vector3D SuperQuadric::getSurfacePositionNoTaper(float u, float v) const {
    Vector3D result;

    result.x = (float)(A*sign(cosf(v)*cosf(u))*pow(fabs(cosf(v)), e1)*powf(fabs(cosf(u)), e2));
    result.y = (float)(B*sign(cosf(v)*sinf(u))*pow(fabs(cosf(v)), e1)*powf(fabs(sinf(u)), e2));
    result.z = (float)(C*sign(sinf(v))*powf(fabs(sinf(v)), e1));
