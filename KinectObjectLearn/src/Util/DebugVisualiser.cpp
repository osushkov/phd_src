...
void DebugVisualiser::renderInfo(std::vector<std::pair<Vector2D,Vector2D> > matched_features,
    ...
    for(unsigned i = 0; i < matched_features.size(); i++){
        CvPoint pos0 = cvPoint((int)matched_features[i].first.x, (int)matched_features[i].first.y);
        CvPoint pos1 = cvPoint((int)matched_features[i].second.x + imgA->width,
                               (int)matched_features[i].second.y);
