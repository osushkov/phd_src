...
static void displayGripPath(std::vector<float> src_joints, std::vector<float> dst_joints, Object *object, Transform arm_to_obj_pose){
	...
	SuperQuadricRenderObject *sq_ro = new SuperQuadricRenderObject(SuperQuadric(1.0f, 1.0f, 2.0f, 2.0f, 2.0f));
...
Transform Control::pickUpObject(Object *obj, bool &success){
    ...
    float grip_size = calculateGripSize(grip_data, obj, cur_pose);
        
    float grip_size = calculateGripSize(grip_data, obj, cur_pose) + 1.0f;
...
bool Control::moveToExperiment(Object *obj, Transform arm_to_object, DropExperiment *exp, float error_threshold){



    float error;

    std::vector<float> experiment_joints = 
        ObjectExperimentTransfer::transferObject(arm_to_object, exp->getUpVector(), exp->getDropHeight(), last_grip_data.arm_joints, error);
    ...
    if((middle + 15.0f*to_gripper0).z < 12.8f || (middle + 15.0f*to_gripper1).z < 12.8f){
        return false;
    }
...
void Control::safeMoveToJoints(std::vector<float> joints){
    ...
    stage1_joints[0] = 26.31f;

    stage1_joints[1] = 0.0f;
    stage1_joints[5] = 0.0f;

    
	std::vector<float> stage2_joints = joints;
	stage2_joints[0] = 26.31f;

	stage2_joints[1] = 0.0f;
