...
Recorder::Recorder(std::string dir_path, Camera *rc, bool record_arm) :
    dir_path(dir_path), rc(rc), record_arm(record_arm) {
...
void* Recorder::recorderThread(void *thread_args){
    ...
    while(recorder->am_recording){
        ...
        if(!recorder->rc->getImage(new_frame->left_buffer, new_frame->right_buffer, false)){
            break;
        }

        if(recorder->record_arm){
            new_frame->arm_joints = Arm::getArm()->requestCurrentAngles();
        }
...
void* Recorder::writerThread(void *thread_args){
    ...
    while(recorder->am_recording || recorder->buffer.size() > 0){
        ...
        record_file << "frame: " << num_written_frames << " " << left_filename << " "
                    << right_filename << " " << timer.getNumElapsedSeconds();

        if(recorder->record_arm){
            record_file << " " << arm_joints.size();
            for(unsigned i = 0; i < arm_joints.size(); i++){
                record_file << " " << arm_joints[i];
            }
        }
