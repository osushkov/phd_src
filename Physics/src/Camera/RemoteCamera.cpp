...
RemoteCamera::RemoteCamera(std::string camera_address, int port) :
    ...
    new_frame(0), new_frame_consumed(0), frame_num(0), thread_alive(0), active(true) {
...
bool RemoteCamera::getImage(std::vector<unsigned char> &lbuffer,
...
void* RemoteCamera::update(void *thread_arg){
    ...
    std::vector<unsigned char> tmp_left_buffer, tmp_right_buffer;
    ...
    while(bytes_received > 0 && arg->remote_camera->active){
        ...
        if(total_received == (int)buffer_size) {
            ...
            if(cur_camera == 0){
                for(unsigned i = 0; i < buffer_size; i++){
                    tmp_left_buffer[i] = (unsigned char)img_buffer[i];
                }
            }
            else{
                for(unsigned i = 0; i < buffer_size; i++){
                    tmp_right_buffer[i] = (unsigned char)img_buffer[i];
                }
            }
    ...
    delete[] img_buffer;
    arg->remote_camera->thread_alive.signal();
