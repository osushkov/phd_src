...
void ExperimentCoordinator::initialise(std::vector<ObjectPhysModel> phys_models, std::vector<Experiment*> experiments){
    ...
    initialiseRamp(15.0f*M_PI/180.0f, 0.0f);
    ...
    */
    ...
    real_model = new ObjectPhysModel(Vector3D(5.0f, 3.0f, 1.5f), 2.0f, wheel_params, stub_params);
    ...
    if(!ExperimentResultClassifier::instance().isBuilt()){
        ...
        for(unsigned i = 0; i < all_experiments.size(); i++){
            for(unsigned j = 0; j < phys_models.size(); j++){
                ...
                WheeledBoxPhysicsObject *phys_object = phys_models[j].getPhysicsObject();
                WheeledBoxRenderObject *render_object = NULL; //phys_models[j].getRenderObject();
...
void ExperimentCoordinator::initialiseRamp(float ramp_theta, float ramp_xend){
    ...
    ramp_rigid_body->setFriction(0.6f);
...
/*
    ...
    float a0 = acosf(target_experiment->getUpVector().dotProduct(Vector3D(0.0f, 0.0f, 1.0f))) * 180.0f/M_PI;
    float a1 = acosf(target_experiment->getUpVector().dotProduct(Vector3D(0.0f, 0.0f, -1.0f))) * 180.0f/M_PI;
    std::cout << "angles: " << a0 << " " << a1 << std::endl;
