...
IplImage* convertToGray32( const IplImage* );
...
IplImage*** buildGaussPyr(const IplImage* base, int octvs,
    ...
    float sig[_intvls+3], sig_total, sig_prev, k;
...
struct feature* interpExtremum(const IplImage*** dog_pyr, int octv, int intvl,
    ...
    feat = newFeature();
    ddata = feat_detection_data( feat );
    feat->img_pt.x = feat->x = (c + xc) * powf(2.0f, octv);
    feat->img_pt.y = feat->y = (r + xr) * powf(2.0f, octv);
    ddata->r = r;
    ddata->c = c;
    ddata->octv = octv;
    ddata->intvl = intvl;
...
float* oriHist(const IplImage* img, int r, int c, int n, int rad, float sigma){
    float* hist;
    float mag, ori, w, exp_denom, PI2 = CV_PI * 2.0f;
    int bin, i, j;
