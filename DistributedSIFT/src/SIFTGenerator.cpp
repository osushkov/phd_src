...
SIFTGenerator::SIFTGenerator(std::string filename, bool read_from_file, 
    ...
    lock(1), start_semaphore(0), end_semaphore(0) {
...
void* SIFTGenerator::producerThreadLive(void *thread_arg){
    ...
    while(1){
        ...
        SIFTGeneratorStartData cur_data;

        parent->lock.wait();
        cur_data = parent->start_queue.front();
        ...
        IplImage* img_left = Common::imageFromBuffer(cur_data.left_cam_buffer, parent->cam_width, parent->cam_height);
        IplImage* img_right = Common::imageFromBuffer(cur_data.right_cam_buffer, parent->cam_width, parent->cam_height);
