...
void SceneRenderer::initialise(bool independent){
    if(!is_initialised){
...
void SceneRenderer::signal(void){
    signal_sem0.signal();
    signal_sem1.wait();
}
...
void* SceneRenderer::renderThread(void *thread_arg){
    ...
    while(!scene_renderer->done){
        ...
        if(!scene_renderer->is_independent){
            scene_renderer->signal_sem0.wait();
            scene_renderer->signal_sem1.signal();
        }
    }
...
void SceneRenderer::drawFloor(float height, unsigned cell_dim){
    float xrange[] = {-40.0f, 40.0f};
    float yrange[] = {0.0f, 70.0f};
