...
std::vector<float> NelderMead::optimise(OptimisableFunction *target, unsigned np,
    ...
    for(unsigned iter = 0; iter < max_evals; iter++){
        cur_iters = iter;
        if(last_lowest_iter > 0 && (iter - last_lowest_iter) > max_evals/4) {
        ...
        for(unsigned i = 0; i < centre_of_mass.size(); i++){
            assert(centre_of_mass[i] == centre_of_mass[i]);
            assert(centre_of_mass[i] >= -0.0001f && centre_of_mass[i] <= 1.0001f);
        }
        ...
        sort(simplex_vertex.begin(), simplex_vertex.end(), comp);
        for(unsigned i = 0; i < simplex_vertex.size(); i++){
            assert(simplex_vertex[i].value == simplex_vertex[i].value);
        }
        ...
        if(simplex_vertex.front().value < lowest_value || iter == 0){
            ...
            if(lowest_value <= target_fitness){
                break;
            }
        }
    }

    return simplex_vertex.front().coord;
}
...
std::vector<float> NelderMead::centreOfMass(const std::vector<NelderMead::SimplexVertex> &vertices, unsigned num_vertices){
    ...
    for(unsigned i = 0; i < num_vertices; i++){
        ...
        for(unsigned j = 0; j < vertices.at(i).coord.size(); j++){
            assert(vertices.at(i).coord.at(j) == vertices.at(i).coord.at(j));
            assert(vertices.at(i).coord.at(j) >= -0.0001f && vertices.at(i).coord.at(j) <= 1.0001f);
            assert(vertices.at(i).value == vertices.at(i).value);
            assert(j < result.size());

            result.at(j) += vertices.at(i).coord.at(j) * vertices.at(i).value;

            if(result.at(j) != result.at(j)){
                std::cout << num_vertices << " " << num_params << std::endl;
                std::cout << result.size() << " " << j << std::endl;
                for(unsigned k = 0; k < vertices.at(i).coord.size(); k++){
                    std::cout << vertices.at(i).coord.at(k) << " " << vertices.at(i).value << std::endl;
                }
            }

            assert(result.at(j) == result.at(j));
            assert(result.at(j) >= 0.0f);
        }
...
std::vector<NelderMead::SimplexVertex> NelderMead::createInitialSimplex(OptimisableFunction *target, unsigned dim){
    ...
    for(unsigned i = 0; i < dim; i++){
        if(prime_vertex.coord.at(i) < 0.0f){
            prime_vertex.coord.at(i) = 0.0f;
        }
        else if(prime_vertex.coord.at(i) > 1.0f){
            prime_vertex.coord.at(i) = 1.0f;
        }
    }
    ...
    for(unsigned i = 0; i < dim; i++){
        ...
        new_vertex.coord.at(i) += 0.05f; // TODO define this
    ...
    for(unsigned i = 0; i < result.size(); i++){
        result.at(i).value = target->eval(result.at(i).coord, 0.0f);
        assert(Common::isValidValue(result.at(i).value)); //big number
    }
...
NelderMead::SimplexVertex NelderMead::reflectedVertex(OptimisableFunction *target, NelderMead::SimplexVertex vertex,
    ...
    for(unsigned i = 0; i < result.coord.size(); i++){
        assert(result.coord[i] == result.coord[i]);
        assert(result.coord[i] >= -0.0001f && result.coord[i] <= 1.0001f);
    }

    result.value = target->eval(result.coord, (float)cur_iters/(float)iter_limit);
    assert(Common::isValidValue(result.value));
