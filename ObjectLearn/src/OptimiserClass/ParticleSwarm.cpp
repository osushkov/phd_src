  ...
  public:
    ...
    void performTask(void *task_data, unsigned rank, unsigned size){
        ...
        for(unsigned i = start_index; i < end_index; i++){
            ...
            float iter_frac = (float)pswarm->cur_iter/(float)pswarm->iter_limit;
            float current_particle_value =
                    pswarm->func_target->eval(pswarm->swarm.at(i).position, iter_frac);
            if(current_particle_value < pswarm->swarm.at(i).best_known_value){
                ...
                if(current_particle_value < pswarm->best_global_value){
                    ...
                    std::cout << "new best: " << current_particle_value << std::endl;
                }
                pswarm->lock.signal();
            }
        }
...
ParticleSwarm::Particle ParticleSwarm::createInitialParticle(OptimisableFunction *target){
    ...
    np.best_known_value = target->eval(np.position, 0.0f);
