...
*/
    ...
    if(best_result >= 5.0f){
        ...
        for (unsigned i = 0; i < best_matches.size(); i++) {
            ...
            matched_scene_features.push_back(best_matches[i].index0);

            std::pair<Vector2D,Vector3D> new_perspective_pair;
            new_perspective_pair.first = Vector2D(scene_features[best_matches[i].index0].feature_left.x,
                                                  scene_features[best_matches[i].index0].feature_left.y);
            new_perspective_pair.second = snapshot_features[best_matches[i].index1].position;
