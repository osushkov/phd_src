...
void CorrelatedFrameParser::write(IplImage *left_frame, IplImage *right_frame,
    ...
    assert(left_frame != NULL);
    assert(right_frame != NULL);

    std::stringstream left_frame_filename;
    left_frame_filename << "left_" << cur_output_num << ".png";

    std::stringstream right_frame_filename;
    right_frame_filename << "right_" << cur_output_num << ".png";

    cvSaveImage((path+left_frame_filename.str()).c_str(), left_frame);
    cvSaveImage((path+right_frame_filename.str()).c_str(), right_frame);
    ...
    root_file << cur_output_num << " "
              << left_frame_filename.str() << " "
              << right_frame_filename.str() << " "
...
std::vector<CorrelatedFrame> CorrelatedFrameParser::read(void){
    ...
    while(!in_file.eof()){
        ...
        in_file >> new_frame.left_frame_filename;
        in_file >> new_frame.right_frame_filename;
...
void CorrelatedFrameParser::writeOutFeatures(std::ofstream &out_file,
    ...
    for(unsigned i = 0; i < features.size(); i++){
        ...
        write_feature(out_file, features[i].feature_left);
        write_feature(out_file, features[i].feature_right);
...
void CorrelatedFrameParser::readInFeatures(std::ifstream &in_file,
    ...
    for(unsigned i = 0; i < num_features; i++){
        ...
        read_feature(in_file, new_feature.feature_left);
        read_feature(in_file, new_feature.feature_right);
