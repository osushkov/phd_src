...
bool Geometry::linePlaneIntersect(Vector3D l0, Vector3D l1, Vector3D normal, float d, float &t){
    Vector3D line_vec = l1 - l0;
    float denominator = line_vec.dotProduct(normal);
    float numerator = -d - l0.dotProduct(normal);
...
float Geometry::linePointDistance(Vector2D line1, Vector2D line2, Vector2D point){
    return linePointDistance(Vector3D(line1.x, line1.y, 0.0f),
                             Vector3D(line2.x, line2.y, 0.0f),
                             Vector3D(point.x, point.y, 0.0f));
}

float Geometry::linePointDistanceT(Vector2D line1, Vector2D line2, Vector2D point){
    return linePointDistanceT(Vector3D(line1.x, line1.y, 0.0f),
                              Vector3D(line2.x, line2.y, 0.0f),
                              Vector3D(point.x, point.y, 0.0f));
}
...
float Geometry::minAngleDistance(float angle0, float angle1){
    ...
    while(angle0 >= 2.0f*M_PI){ angle0 -= 2.0f*M_PI; }
    while(angle1 >= 2.0f*M_PI){ angle1 -= 2.0f*M_PI; }
...
std::vector<Vector2D> Geometry::getLinePixels(Vector2D start, Vector2D end, int inc){
    Vector2D lvec = end - start;
    float num_pixels_spanned = lvec.length();

    std::vector<Vector2D> result;
    for(unsigned i = 0; i < (int)(num_pixels_spanned+0.5f); i += inc){
        Vector2D pos = start + (i/num_pixels_spanned)*lvec;
        pos.x = floor(pos.x);
        pos.y = floor(pos.y);
