...
bool Console::parseCommand(std::string command){
    ...
    else if(tokens[0] == "correlate" || tokens[0] == "c"){
        correlateObjectsCommand(tokens);
    }
    ...
    else if(tokens[0] == "silhouette_object" || tokens[0] == "so"){
        silhouetteObject(tokens);
    }
    else{
...
void Console::correlateObjectsCommand(std::vector<std::string> args){
    if(args.size() != 2){
        std::cout << "Incorrect number of arguments" << std::endl;
        return;
    }

    MainController::instance().correlateObjects(args[1]);
}
...
void Console::generateSIFTCommand(std::vector<std::string> args){
    ...
    fixPath(path);
    MainController::instance().generateSIFT(path);
}
...
void Console::silhouetteObject(std::vector<std::string> args){
    if(args.size() != 2){
        std::cerr << "Incorrect number of arguments" << std::endl;
        std::cerr << "Expected: silhouetteObject object_name" << std::endl;
        return;
    }

    MainController::instance().silhouetteObject(args[1]);
}
