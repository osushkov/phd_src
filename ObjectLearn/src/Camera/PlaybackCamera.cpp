...
struct ImageLoadWorkerData {
	ImageLoadWorkerData(std::string filename, IplImage **img) :
...
class ImageLoadWorker : public ParallelExecutor {
  public:
    ...
    void performTask(void *task_data, unsigned rank, unsigned size){
    	ImageLoadWorkerData *data = (ImageLoadWorkerData *)task_data;
    	*(data->img) = cvLoadImage(data->filename.c_str());
    }
};
...
void* PlaybackCamera::update(void *thread_arg){
    ...
    ImageLoadWorker worker_thread;
    Util::Semaphore task_sem;

    ImageLoadWorkerData left_worker_data(left_frame_filename, &img_left);
    ImageLoadWorkerData right_worker_data(right_frame_filename, &img_right);

    ParallelTask task0(&worker_thread, &left_worker_data, &task_sem);
    ParallelTask task1(&worker_thread, &right_worker_data, &task_sem);
    ...
    while(!playback_camera->video_file.eof()){
        ...
        left_worker_data.filename = left_frame_filename;
        right_worker_data.filename = right_frame_filename;

        pserver.executeParallelTask(task0);
