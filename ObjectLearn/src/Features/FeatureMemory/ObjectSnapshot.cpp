...
float ObjectSnapshot::matchScene(const std::vector<StereoFeature> &scene_features,
                                 std::set<unsigned> &scene_matches,
                                 bool hi_accuracy){
    ...
    for (unsigned i = 0; i < num_triplets_to_check; i++) {
        ...
        if (score >= 5.0f) {
            for(unsigned j = 0; j < cur_matches.size(); j++){
                scene_matches.insert(cur_matches[j].index0);
            }
        }
    }
...
std::vector<float> ObjectSnapshot::buildDistanceMatrix(const std::vector<StereoFeature> &scene_features){
...
std::vector<FeatureMatchPair> ObjectSnapshot::buildMatchPairs(const std::vector<StereoFeature> &scene_features,
                                                              std::vector<float> &distance_matrix){
...
void ObjectSnapshot::setParams(bool hi_accuracy){
    if(hi_accuracy){
        ...
        params.max_fori_diff = 0.5f;
        params.min_fpmatch   = 0.85f;

        params.max_triplets       = 25;
        params.max_triplets_check = 20;

        params.min_triplet_score = 0.8f;
        params.triplet_dist_sd   = 0.25f;
        params.triplet_ori_sd    = 0.25f;
    }
    else{
        params.num_triplets_check = -1;
