...
x <- ponyx
y <- (caty + cowy + dogy + ponyy + shipy + skydiversy)/1.88
...
plot(x, y, type = 'o', ylim = c(0, 80), lty=4, pch=3, xlab="Training Frames", ylab="Percent of Object Features", main="Automatic Segmentation (Translation) vs Manual Segmentation");
par(new=TRUE)


legend("right", legend = c("Correct Features", "Incorrect Features", "Baseline Correct Features", "Baseline Incorrect Features"), text.width = strwidth("Baseline Incorrect Features"), xjust = 1, yjust = 1, lty = c(1,2,3,4), pch = c(0,1,2,3), bty = "n", cex = 0.8, title = "")
