...
std::pair<Vector3D, Vector3D> Geometry::perpedicularBasis(Vector3D vec){
    vec.normalise();

    Vector3D v0(1.0f, 0.0f, 0.0f);
    if(fabs(1.0f - vec.dotProduct(v0)) < 0.1f){
        v0 = Vector3D(0.0f, 1.0f, 0.0f);
    }

    std::pair<Vector3D, Vector3D> result;

    result.first = vec.crossProduct(v0);
    result.second = result.first.crossProduct(vec);
