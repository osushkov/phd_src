...
std::vector<ObjectLegoModel> MainController::createPossiblePhysModels(void){
    float left_turn_dirs[] = {0, -1};
    float left_run_dirs[] = {0, 1};
    float right_turn_dirs[] = {0, -1};
    float right_run_dirs[] = {0, 1};

    std::vector<std::pair<Vector3D,Vector3D> > left_sensor_poses;
    left_sensor_poses.push_back(std::pair<Vector3D,Vector3D>(Vector3D(-5.0f, 6.0f, -1.5f), Vector3D(-0.707f, 0.0f, -1.0)));
    //left_sensor_poses.push_back(std::pair<Vector3D,Vector3D>(Vector3D(-4.0f, 8.5f, -1.5f), Vector3D(0.0f, 0.707f, -1.0)));

    std::vector<std::pair<Vector3D,Vector3D> > right_sensor_poses;
    right_sensor_poses.push_back(std::pair<Vector3D,Vector3D>(Vector3D(-5.0f, -6.0f, -1.5f), Vector3D(-0.707f, 0.0f, -1.0)));
    //right_sensor_poses.push_back(std::pair<Vector3D,Vector3D>(Vector3D(-4.0f, -8.5f, -1.5f), Vector3D(0.0f, -0.707f, -1.0)));
            ...
            for(unsigned k = 0; k < 2; k++){
                for(unsigned l = 0; l < 2; l++){
                    for(unsigned m = 0; m < left_sensor_poses.size(); m++){
                        ...
                        std::cout << left_turn_dirs[i] << " "
                                  << left_run_dirs[j] << " "
                                  << right_turn_dirs[k] << " "
                                  << right_run_dirs[l] << std::endl;
                        left_sensor_poses[m].first.print();
                        right_sensor_poses[m].first.print();
                        std::cout << std::endl;
                        */
...
std::vector<Experiment*> MainController::createAvailableExperiments(void){
    ...
    if(!tryLoadExperiments("data/experiments.dat", result)){
        ...
        const unsigned num_positions = 50;
