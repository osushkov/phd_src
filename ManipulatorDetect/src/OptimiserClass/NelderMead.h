...
class NelderMead : public Optimiser {
  ...
  private:
    ...
    SimplexVertex reflectedVertex(OptimisableFunction *target, SimplexVertex vertex, std::vector<float> rpoint, float reflect_dist=1.0f);
    SimplexVertex contractedVertex(OptimisableFunction *target, SimplexVertex vertex, std::vector<float> cpoint, float contract_ratio=0.5f);
